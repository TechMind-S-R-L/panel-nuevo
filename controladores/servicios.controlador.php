...
class ControladorServicios{
	...
	static public function ctrCrearServicio(){
		...
		if($esSoftware && (empty($_POST["idClienteServicio"]) || empty($_POST["nombreProyectoSoftware"]) || empty($_POST["tipoSoftwareProyecto"]) || $total <= 0 || $montoAdelantoSoftware <= 0 || $montoAdelantoSoftware > $total)){
			echo '<script>swal({type:"error",title:"Complete cliente, proyecto, precio total y adelanto valido",text:"El adelanto debe ser mayor a cero y no puede superar el precio total.",confirmButtonText:"Cerrar"});</script>';
			return;
		}
