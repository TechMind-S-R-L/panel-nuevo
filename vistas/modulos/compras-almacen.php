    ...
    <div class="box">
      ...
      <div class="box-body">
        <table class="table table-bordered table-striped dt-responsive tablas" width="100%">
          <tbody>
            <?php
            $controladorOrdenIngreso = 'ControladorOrdenIngreso';
            $ordenes = class_exists($controladorOrdenIngreso) ? $controladorOrdenIngreso::ctrMostrarOrdenesPendientes() : [];

            foreach ($ordenes as $orden) {
                echo "
                <tr>
                    <td>{$orden['id_orden_ingreso']}</td>
                    <td>{$orden['codigo_producto']}</td>
                    <td>{$orden['unidades']}</td>
                    <td>{$orden['fecha_ingreso']}</td>
                    <td>
                        <button class='btn btn-primary btnIngresarProducto' data-id-orden='{$orden['id_orden_ingreso']}'>Ingresar</button>
                    </td>
                </tr>";
            }
            ?>
            <div id="modalIngresarProducto" class="modal fade" role="dialog">
    <div class="modal-dialog">
        <div class="modal-content">
            <form id="formIngresarProducto">
                <div class="modal-header">
                    <button type="button" class="close" data-dismiss="modal">&times;</button>
                    <h4 class="modal-title">Ingresar Producto</h4>
                </div>
                <div class="modal-body">
                    <input type="hidden" id="idOrdenIngreso" name="idOrdenIngreso">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="categoriaProducto" name="categoriaProducto" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label>Código de Barras</label>
                        <input type="text" id="codigoBarras" name="codigoBarras" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label>Unidades</label>
                        <input type="number" id="unidadesProducto" name="unidadesProducto" class="form-control" required>
                    </div>
                    <div class="form-group">
                        <label>Factura Escaneada</label>
                        <input type="file" id="facturaEscaneada" name="facturaEscaneada" class="form-control">
                    </div>
                </div>
