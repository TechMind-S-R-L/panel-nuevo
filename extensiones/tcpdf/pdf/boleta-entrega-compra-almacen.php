...
function RotatedTextEntregaCompra($pdf, $x, $y, $txt, $angle) {
	$pdf->StartTransform();
...
$almacenero = !empty($compra["id_almacenero_entrega"]) ? ControladorUsuarios::ctrMostrarUsuarios("id", $compra["id_almacenero_entrega"]) : null;
$proveedor = ControladorProveedor::ctrMostrarProveedor("id", $compra["id_proveedor"]);
$productos = json_decode($compra["productos"], true);
$estado = strtoupper(str_replace("_", " ", $compra["estado"]));
...
$pdf->SetAlpha(0.1);
RotatedTextEntregaCompra($pdf, 26, 180, 'ENTREGA ALMACEN', 45);
...
$pdf->SetTextColor(70, 130, 180);
$pdf->Cell(82, 7, 'CONSTANCIA ENTREGA A ALMACEN', 0, 1, 'R');
...
<table style="font-size:10px;" cellspacing="0" cellpadding="4" border="1">
  ...
  <tr>
    <td><b>Mensajero entrega:</b> '.htmlspecialchars($mensajero["nombre"] ?? "Pendiente", ENT_QUOTES, "UTF-8").'</td>
    <td><b>Almacen recibe:</b> '.htmlspecialchars($almacenero["nombre"] ?? "Pendiente de registrar", ENT_QUOTES, "UTF-8").'</td>
  </tr>
  <tr>
    <td><b>Fecha toma mensajero:</b> '.htmlspecialchars($compra["fecha_toma_mensajero"] ?? "Pendiente", ENT_QUOTES, "UTF-8").'</td>
    <td><b>Fecha entrega almacen:</b> '.htmlspecialchars($compra["fecha_entrega_almacen"] ?? "Pendiente", ENT_QUOTES, "UTF-8").'</td>
  </tr>
</table>
<br>
<h3>PRODUCTOS ENTREGADOS A ALMACEN</h3>
<table style="font-size:10px;" cellspacing="0" cellpadding="4" border="1">
...
<br>
<p><b>Uso:</b> Constancia interna de entrega fisica de productos comprados por mensajero y recibidos por almacen.</p>
<br><br><br><br>
<table style="font-size:9px;" cellpadding="8">
  <tr>
    <td align="center">_________________________<br>Firma mensajero</td>
    <td align="center">_________________________<br>Firma almacen</td>
  </tr>
</table>';

$pdf->writeHTML($html, true, false, true, false, '');
if(ob_get_length()){ ob_end_clean(); }
$pdf->Output('boleta-entrega-almacen-'.$compra["codigo"].'.pdf', 'I');
